...
index_to_x = [i * 0.000002 + 0.00876 - 0.011112 for i in indexes]
print('Times  are: %s' % (index_to_x))

for xc in index_to_x:
    plt.axvline(x=xc)

x = [i - 0.011112 for i in x]

print(saty[1176],saty[2701],saty[1176] - saty[2701])


def airy(x_data,fine,lamb):
       return [1.0 / (1 + fine * (math.sin(math.pi * p / lamb)** 2)) for p in x_data]
popt, pcov = curve_fit(airy,x,y,p0=[1500,0.00301])
