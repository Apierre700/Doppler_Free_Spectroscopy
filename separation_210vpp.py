...
index_to_x = [i * 0.000002 + 0.00896 - 0.010719999999999999 for i in indexes]
print('Times  are: %s' % (index_to_x))

for xc in index_to_x:
    plt.axvline(x=xc)

x = [i - 0.010719999999999999 for i in x]

print(saty[880],saty[3126],saty[880] - saty[3126])

def airy(x_data,fine,lamb):
       return [1.0 / (1 + fine * (math.sin(math.pi * p / lamb)** 2)) for p in x_data]
popt, pcov = curve_fit(airy,x,y,p0=[150,0.00448])
